...
export function ProductGallery({
  ...
  return (
    <div className={cn("min-w-0 space-y-4", className)}>
      ...
      {images.length > 1 ? (
        <ul
          className="flex max-w-full list-none gap-2 overflow-x-auto overscroll-x-contain p-0 m-0 pb-1 sm:gap-3"
            ...
            return (
              <li key={`${image}-${index}`} className="shrink-0 snap-start">
                <button
                  ...
                  className={cn(
                    "relative size-11 overflow-hidden rounded-lg border-2 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 sm:size-20",
                    isSelected
                      ? "border-primary"
